...
def rank_drivers(
    ...
    *,
    ...
    add_temporary_point_to_graph(service, graph, node_index, pickup_snapped, USER_POINT_NODE_ID)
    add_temporary_point_to_graph(service, graph, node_index, dropoff_snapped, DROPOFF_POINT_NODE_ID)

    trip_path = service.run_a_star(USER_POINT_NODE_ID, DROPOFF_POINT_NODE_ID, graph=graph, node_index=node_index)
    if not trip_path:
        return {
            "rankedCandidates": [],
            "baselineCandidate": None,
    ...
    for index, candidate in enumerate(ranked_candidates, start=1):
        ...
        candidate["selectionReason"] = build_selection_reason(candidate, baseline_candidate)
...
def clone_base_graph(service: Any) -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    graph = {node_id: [dict(neighbor) for neighbor in neighbors] for node_id, neighbors in service.graph.items()}
...
def get_baseline_nearest_driver(service: Any, drivers: list[dict[str, Any]], pickup_node: Any) -> dict[str, Any] | None:
    ...
    best_driver = min(drivers, key=lambda driver: service_distance(service, driver, pickup_node))
    return {
...
def evaluate_candidate(
    ...
    pickup_path = service.run_a_star(start_node.id, USER_POINT_NODE_ID, graph=graph, node_index=node_index)
    if not pickup_path:
        return None
...
def get_driver_route_start_node(service: Any, driver: dict[str, Any]) -> Any | None:
    nearest_node = service.get_nearest_node(float(driver["lat"]), float(driver["lng"]))
    candidates = []
...
def apply_relative_scores(candidates: list[dict[str, Any]]) -> None:
    distance_values = [candidate["pickupDistanceMeters"] for candidate in candidates]
    distance_min = min(distance_values)
    distance_max = max(distance_values)

    for candidate in candidates:
        candidate["distanceScore"] = inverse_relative_score(candidate["pickupDistanceMeters"], distance_min, distance_max)
        candidate["finalScore"] = (
            (candidate["distanceScore"] * 0.30)
            + (candidate["trafficScore"] * 0.20)
            + (candidate["weatherScore"] * 0.10)
...
def get_eta_minutes(distance_meters: float, speed_kph: float, traffic_ratio: float, weather_multiplier: float) -> int:
    adjusted_speed_kph = max(8, speed_kph * max(traffic_ratio, 0.2)) / max(weather_multiplier, 0.8)
...
def service_distance(service: Any, from_point: Any, to_point: Any) -> float:
    from_lat = float(from_point.lat if hasattr(from_point, "lat") else from_point["lat"])
    from_lng = float(from_point.lng if hasattr(from_point, "lng") else from_point["lng"])
    to_lat = float(to_point.lat if hasattr(to_point, "lat") else to_point["lat"])
    to_lng = float(to_point.lng if hasattr(to_point, "lng") else to_point["lng"])
...
def haversine_distance(from_lat: float, from_lng: float, to_lat: float, to_lng: float) -> float:
    ...
    arc = (
        ...
        + math.cos(start_lat_radians) * math.cos(end_lat_radians) * math.sin(delta_lng / 2) ** 2
    )
